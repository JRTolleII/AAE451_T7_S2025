...
Very recently discovered a new baseline needs to be selected. Sources for
baseline now are sourced from wikipedia, this is subject to change
https://en.wikipedia.org/wiki/McDonnell_Douglas_F-15E_Strike_Eagle#Specifications_(F-15E)
https://en.wikipedia.org/wiki/Thrust-specific_fuel_consumption#Typical_values_of_SFC_for_thrust_engines
%}
...
C_ab = 1.9; C_cr = 0.64; C_lt = 0.64; % SFC from raymer table 3.3 for low bypass turbofan x
...
while (((W_0 > 1.005*W_0t)||(W_0 < 0.995*W_0t))||(count==0))
    ...
    wew0 = fwew0(W_0t,AR,TSLW0,W0S,Ma_max,K_vs); % Finding empty weight x
    %disp(wew0);
    W_e = W_0t * wew0;
    ...
    W_0 = W_e + W_f + W_pay; % New gross weight
